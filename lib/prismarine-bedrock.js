...
function setupRoutes(app, prefix = '') {
  ...
  app.use(compression())
  // Resolve prismarine-viewer path properly using require.resolve
  const viewerPath = path.dirname(require.resolve('prismarine-viewer/package.json'))
  const publicPath = path.join(viewerPath, 'public')
  console.log('[Viewer] Serving static files from:', publicPath)
  app.use('/', require('express').static(publicPath))
}

module.exports = (bot, { viewDistance = 6, firstPerson = false, port = 3000, prefix = '', javaVersion = '1.21.11', stickSensitivity = 1, pitchSensitivity = 0.5 }) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.emit('config', { stickSensitivity, pitchSensitivity })
    sockets.push(socket)
    ...
    function botPosition() {
      if (bot.self.position.x === pos.x && bot.self.position.y === pos.y && bot.self.position.z === pos.z && bot.self.yaw == yaw) {
        return
      }
